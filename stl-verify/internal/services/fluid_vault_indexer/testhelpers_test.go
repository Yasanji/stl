...
// stubFluidRepo is an in-memory FluidVaultRepository for unit tests. UpsertVaults
...
func (s *stubFluidRepo) UpsertVaults(_ context.Context, _ pgx.Tx, vaults []*entity.FluidVault) (map[common.Address]int64, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
	if s.upsertErr != nil {
